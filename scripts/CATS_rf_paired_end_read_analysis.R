#!/usr/bin/env Rscript
#Paired-end read analysis script
...
OUT_PREF <- ext_args[8]
IMP_WITHIN_READ_PROP_BREAKS <- as.numeric(unlist(strsplit(ext_args[9], ",")))
DIFF_TR_PAIR_READ_PROP_BREAKS <- as.numeric(unlist(strsplit(ext_args[10], ",")))
ALPHA_COMP_FACT <- as.numeric(ext_args[11])
BETA_COMP_FACT <- as.numeric(ext_args[12])
FRAG_TR_BRIDGE_N <- as.numeric(ext_args[13])

#Cutting a numerical variable into intervals (function)
