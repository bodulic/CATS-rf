...
import_data <- function(import_regex, select_cols = NA) {
 ...
 } else if (import_regex %chin% c("general_statistics_table.tsv", "assembly_score_summary.tsv", "coverage_and_accuracy_analysis_summary.tsv", "paired_end_read_analysis_summary.tsv")) {
  for (i in 1 : length(files_in_directory)) {
   if (i == 1) {
...
process_filenames <- function(dt_to_process, file_regex) {
 ...
 dt_to_process[, "occurrence_N" := rleid(full_name), by = "transcriptome"]
 dt_to_process[, "full_name" := NULL]
 dt_to_process[occurrence_N != 1, "transcriptome" := paste(transcriptome, occurrence_N, sep = ".")]
 dt_to_process[, "occurrence_N" := NULL]
...
analyse_numerical_variable <- function(dt_to_analyse_filename, variable_to_plot, plot_type, plot_axis_title_filename) {
 ...
 if(plot_type == "raincloud") {
  ...
  variable_rp <- ggplot(data = dt_to_analyse, aes(x = transcriptome, y = .data[[variable_to_plot]], fill = transcriptome)) +
   ...
   theme_bw() +
   scale_fill_manual(values = RP_COLORS[TRANSCRIPTOME_N : 1]) +
  ...
  variable_hi <- ggplot(data = dt_to_analyse, aes(x =.data[[variable_to_plot]], fill = transcriptome)) +
   ...
   scale_x_sqrt(limits = c(NA, plotted_value_max)) +
...
HI_HEIGHT  <- c(5.5, 7, 9, 10, 12.8, 14, 15, 17, 19, 21, 23, 25, 26, 27, 28)[findInterval(TRANSCRIPTOME_N, c(0, 6, 11, 18, 26, 31, 41, 51, 60, 65, 70, 88, 95, 101, 105, Inf))]
...
```
**Figure 6.** Maximum uncovered region length per transcript distribution (in bp).
<br>
<br>
...
```
**Figure 9.** Low-coverage region length distribution (in bp).
<br>
<br>
...
```
**Figure 15.** Low-accuracy region length distribution (in bp).
<br>
<br>
...
fidelity_integrity_statistics <- import_data("paired_end_read_analysis_summary.tsv")
